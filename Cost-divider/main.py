...
try:
    ...
    for less in less_payers:
        while less.payment > 0 and status:
            for more in more_payers:
                if more.payment == 0:
                    more_payers.remove(more)
                    if len(more_payers) == 0:
                        status = False
                    continue
                    ...
                    less.payment = round(less.payment - more.payment,2)
                    more.payment = 0
                else:
                    if less.name in paying_details:
                    ...
                    more.payment = round(more.payment - less.payment,2)
                    less.payment = 0
                    break
